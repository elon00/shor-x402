...
async function startServer() {
  ...
  // Helper to verify x402 authorization header
  const checkX402Payment = (
    ...
    // Expected header format: "x402-algo <token>" or "Bearer <token>"
    if (authHeader && (authHeader.startsWith('x402-algo ') || authHeader.startsWith('Bearer '))) {
      return true; // Authorized!
    }

    // Otherwise emit true standard HTTP 402 Payment Required!
    const nonce = `x402_nonce_${Math.random().toString(36).substring(2, 12)}`;
    const expiresAt = Date.now() + 10 * 60 * 1000; // 10 minutes

    res.setHeader('WWW-Authenticate', `x402 realm="shor-agent-commerce", network="algorand", asset="USDC", asset_id=31566704, amount="${costUsdc.toFixed(6)}", recipient="${recipient}", nonce="${nonce}", pqc="ML-DSA-65"`);
    ...
    res.setHeader('X-402-Asset', 'USDC (ASA 31566704)');
  ...
  // --- Live Paid Digital Service Endpoints ---

  // 1. Weather & Planetary Radar API ($0.002 USDC)
  app.get('/api/services/weather', (req, res) => {
    const isPaid = checkX402Payment(
      req,
      res,
      'srv-weather-hyperlocal',
      0.002,
      'WEATHR7QW8V9E3LK2X64MPN56YZAQ1B2C3D4E5F6G7H8J9K0L1M2N3P4Q5'
    );
    if (!isPaid) return;
  ...
  // 2. Quantum Neural Inference Engine API ($0.015 USDC)
  app.get('/api/services/quantum-inference', (req, res) => {
    const isPaid = checkX402Payment(
      req,
      res,
      'srv-quantum-inference',
      0.015,
      'QUANTUM777AAABBBCCCDDDEEEFFFGGGHHHJJJKKKLLLMMMNNNPPPRRRSSST'
    );
    if (!isPaid) return;
  ...
  // 3. Distributed Satellite Compute & GIS Matrix ($0.050 USDC)
  app.get('/api/services/satellite-compute', (req, res) => {
    const isPaid = checkX402Payment(
      ...
      'srv-hpc-satellite',
      0.05,
      'ORBITAL999GIS444X888Z222M111K333W555T777V666B000N999P888Q7'
    );
    if (!isPaid) return;
  ...
  // 4. Market Intelligence Feed ($0.008 USDC)
  app.get('/api/services/market-intelligence', (req, res) => {
    const isPaid = checkX402Payment(
      ...
      0.008,
      'APEXALPHA888999111222333444555666777888999AAABBBCCCDDDEEEF'
    );
    if (!isPaid) return;
  ...
  // 5. True Quantum Entropy Generator ($0.001 USDC)
  app.get('/api/services/pqc-entropy', (req, res) => {
    const isPaid = checkX402Payment(
      ...
      'srv-pqc-entropy',
      0.001,
      'SHORLABS1234567890ABCDEFGHJKLMNPQRSTUVWXYZ234567890ABCDEFGH'
    );
    if (!isPaid) return;
  ...
  // --- x402 Facilitator Settlement Verification Endpoint ---
  app.post('/api/x402/verify-payment', (req, res) => {
    const { txId, sender, receiver, amount, asset, nonce, serviceId } = req.body;
  ...
  app.post('/api/gemini/plan-and-reason', async (req, res) => {
    ...
    if (!userGoal) {
      return res.status(400).json({ error: 'Missing userGoal' });
    }
...
Only output the valid JSON block without markdown backticks.`;
    ...
    // Fallback deterministic planner if Gemini API key not present or error occurred
    ...
    res.json({
      ...
      steps: [
        ...
        },
        {
          ...
          state: 'S4_EVALUATE',
          reasoning: 'Solving H(x) = αC + βL + γR + δQ + λP to select optimal endpoint.',
          costUsdc: 0,
        },
        {
          stepNumber: 3,
